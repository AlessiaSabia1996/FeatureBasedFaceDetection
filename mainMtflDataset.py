...
def main():
    ...
    # Carico il dataset
    # dataset = np.loadtxt('feature_files_3.csv', delimiter=',')

    # Test addestramento rete neurale
    # looTrainNeuralNetwork(dataset, epochs=100)
    # kfoldTrainNeuralNetwork(dataset, epochs=100)

    # Test uso modello rete neurale
    # model = tf.keras.models.load_model("modelloReteNeurale-LeaveOneOut.h5")
    # predictions = model.predict([[0.6488888888888888,0.6444444444444445,1.0,0.8177777777777778]],  # Occorre matrice
                                # verbose=0)
    ...
    carmine_path = getMtflDatasetPath("carmine") + "images/"
    labelsMat = np.loadtxt(carmine_path + '/labels.txt', delimiter=' ', dtype='str')
    ...
    for images in os.listdir(global_path):
        ...
        coordinates = []
        for label in labelsMat:
            if label[0] == images:
                coordinates = [int(float(label[2])), int(float(label[7])), int(float(label[1])), int(float(label[6]))]
                break
                        ...
                        if mean > 3:  # mean != 0:
                            ...
                            if len(eyes) > 0:
                                ...
                                normalized_coord = [top_left_coord[1] / max_value,
                                                    ...
                                                    bottom_right_coord[0] / max_value, isFace]

                                # with open('trainingDatasets/feature_files_MTFL.csv', 'a+', newline='') as f:
                                #    writer_obj = writer(f)
